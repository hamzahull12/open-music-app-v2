...
class PlaylistsService {
  ...
  async getPlaylistActivities(playlistId) {
    const query = {
      text: 'SELECT u.username AS username, s.title AS title, psa.action AS action, psa.time AS time FROM playlist_song_activities psa LEFT JOIN users u ON psa.user_id = u.id LEFT JOIN songs s ON psa.song_id = s.id WHERE psa.playlist_id = $1',
      values: [playlistId],
    };
  ...
  async deletePlaylistSongActivities(playlistId, songId, credentialId) {
    const id = `activities-${nanoid(16)}`;
    const action = 'delete';
    const time = new Date().toISOString();
